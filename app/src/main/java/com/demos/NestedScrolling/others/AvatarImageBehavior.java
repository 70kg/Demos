package com.demos.NestedScrolling.others;
...
public class AvatarImageBehavior extends CoordinatorLayout.Behavior<CircleImageView> {
    ...
    public boolean onDependentViewChanged(CoordinatorLayout parent, CircleImageView child, View dependency) {
        ...
        final int maxScrollDistance = (int) (mStartToolbarPosition);//516
    ...
    private void maybeInitProperties(CircleImageView child, View dependency) {
        if (mStartYPosition == 0)//516
            mStartYPosition = (int) (dependency.getY());

        if (mFinalYPosition == 0)//84
            mFinalYPosition = (dependency.getHeight() / 2);

        if (mStartHeight == 0)//360
            mStartHeight = child.getHeight();

        if (mStartXPosition == 0)//540
            mStartXPosition = (int) (child.getX() + (child.getWidth() / 2));

        if (mFinalXPosition == 0)//96
        ...
        if (mStartToolbarPosition == 0)//516
            mStartToolbarPosition = dependency.getY();

        if (mChangeBehaviorPoint == 0) {//0.30555
